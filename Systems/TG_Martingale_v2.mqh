...
//|             ✅ FIXED: Close GridSlicer on Cycle End              |
//+------------------------------------------------------------------+
...
public:
   ...
   //+------------------------------------------------------------------+
   //| Close Cycle (FIXED:  Now closes GridSlicer positions too!)       |
   //+------------------------------------------------------------------+
   bool CloseCycle()
   {
      if(!m_state.IsCycleActive()) return false;
      ...
      // 1. ✅ Close Martingale positions
      SPositionInfo mart_positions[];
      int mart_count = (mode == MODE_BUY) ? 
                       m_scanner.GetMartingaleBuyPositions(mart_positions) : 
                       m_scanner.GetMartingaleSellPositions(mart_positions);
      
      int closed_mart = 0;
      for(int i=0; i<mart_count; i++) {
         if(m_trade.PositionClose(mart_positions[i]. ticket)) closed_mart++;
      }
      
      // 2. ⭐ FIXED: Close GridSlicer positions
      SPositionInfo gs_positions[];
      int gs_count = m_scanner.GetGridSlicerPositions(gs_positions);
      
      int closed_gs = 0;
      for(int i=0; i<gs_count; i++) {
         if(m_trade.PositionClose(gs_positions[i].ticket)) closed_gs++;
      }
      
      // Log hasil closing
      if(m_logger != NULL) {
         m_logger.Info(StringFormat("🎯 Cycle Closed: %d Martingale + %d GridSlicer positions", 
                                    closed_mart, closed_gs));
      }
