...
private:
   ...
   bool CloseTicket(ulong ticket, string reason)
   {
      ...
      {
         // Log dipindahkan ke level strategi utama agar tidak spam, atau log info penting saja
         return true;
      }
      return false;
   }
...
public:
   ...
   void ProcessGlobalBasketTrailing(double current_profit)
   {
      ...
      if(m_basket_active)
      {
         ...
         // Safety: Pastikan exit level minimal di atas break even atau target minimum
         // Di sini kita ikuti logic murni: HWM - Step.
   ...
   void CloseAllPositions(string reason)
   {
      ...
      for(int i = total - 1; i >= 0; i--)
      {
         ...
         if(ticket > 0)
         {
            if(m_magic.IsMagicOurs(PositionGetInteger(POSITION_MAGIC)))
            {
